...
public class AdminReportQueryRepository {
    ...
    private void bindParams(TypedQuery<?> q, ReportSearchCond cond) {
        if (hasText(cond.status()))     q.setParameter("status", Report.Status.valueOf(cond.status()));
        if (hasText(cond.reason()))     q.setParameter("reason", Report.Reason.valueOf(cond.reason()));
        if (hasText(cond.targetType())) q.setParameter("targetType", Report.TargetType.valueOf(cond.targetType()));
